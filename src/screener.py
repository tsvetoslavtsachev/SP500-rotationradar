"""
Universe Screener — пълни метрики за всички акции.

От 6 години дневни adj close цени изчислява:
  Returns:    1M, 3M, 6M, YTD, 1Y, 3Y, 5Y
  Risk:       Vol 1Y/3Y, Sharpe 1Y/3Y, MaxDD 1Y/3Y/5Y, Calmar 3Y
  Position:   52w high distance, 52w low distance, days since 52w high
  Beta:       trailing 1Y vs equal-weight SP500 benchmark

Резултатите захранват "📊 Universe Screener" tab-а.

Метричните функции (returns/risk/position/beta + size_bucket) идват от ВЕНДОРНАТИЯ
shared core `screener_core.py` (байт-идентичен със STOXX600-rotationradar). Тук
остава само US-специфичното: market-cap праговете + build_screener изходната схема.
"""

from __future__ import annotations

import pandas as pd

# DAYS_1Y/3Y/5Y и _max_drawdown се внасят и за tests/test_screener.py, който ги
# импортва от src.screener (re-export на споделеното ядро).
from src.screener_core import (
    DAYS_1M,
    DAYS_1Y,
    DAYS_3Y,
    DAYS_5Y,
    _max_drawdown,
    compute_betas,
    compute_position_metrics,
    compute_returns,
    compute_risk_metrics,
    size_bucket,
)

# Size buckets от market cap (US universe). Праговете са US-специфични — STOXX 600
# ползва ETF weight % прагове (виж STOXX screener.py); самата bucket логика е обща.
LARGE_CAP_THRESHOLD = 50e9
MID_CAP_THRESHOLD = 10e9


def _size_bucket(market_cap: float | None) -> str | None:
    """US wrapper: market cap → size bucket чрез споделения size_bucket."""
    return size_bucket(market_cap, LARGE_CAP_THRESHOLD, MID_CAP_THRESHOLD)


def build_screener(
    prices_df: pd.DataFrame,
    sector_map: dict[str, str] | None = None,
    industry_map: dict[str, str] | None = None,
    name_map: dict[str, str] | None = None,
    market_caps: dict[str, float] | None = None,
) -> pd.DataFrame:
    """
    Изгражда screener DataFrame с по един ред на акция и пълни метрики.
    """
    sector_map = sector_map or {}
    industry_map = industry_map or {}
    name_map = name_map or {}
    market_caps = market_caps or {}

    betas = compute_betas(prices_df)

    rows = []
    for ticker in prices_df.columns:
        prices = prices_df[ticker].dropna()
        if len(prices) < DAYS_1M + 1:
            continue
        row = {
            "ticker": ticker,
            "name": name_map.get(ticker),
            "sector": sector_map.get(ticker),
            "industry": industry_map.get(ticker),
            "market_cap": market_caps.get(ticker),
            "size_bucket": _size_bucket(market_caps.get(ticker)),
            "beta_1y": float(betas[ticker]) if ticker in betas.index else None,
        }
        row.update(compute_returns(prices))
        row.update(compute_risk_metrics(prices))
        row.update(compute_position_metrics(prices))
        rows.append(row)

    return pd.DataFrame(rows)

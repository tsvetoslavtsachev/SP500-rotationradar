...
def test_size_bucket_thresholds():
    assert _size_bucket(100e9) == "Large"
    assert _size_bucket(50e9) == "Large"
    assert _size_bucket(20e9) == "Mid"
    assert _size_bucket(10e9) == "Mid"
    assert _size_bucket(5e9) == "Small"
    assert _size_bucket(None) is None
    assert _size_bucket(0) is None
...
def test_build_screener_full_pipeline():
    ...
    market_caps = {f"T{i}": (100e9 if i < 2 else 5e9) for i in range(5)}
    ...
    assert len(df) == 5
    expected_cols = {
        "ticker", "sector", "market_cap", "size_bucket",
